...
<?php
  if($_SESSION['login_success']==true){ 
	...
	$sql="SELECT * FROM defective_items WHERE Status LIKE 'pending' ORDER BY id DESC";
    
	
	$records=mysqli_query($con, $sql) or die('error getting data');
	$heading = "List of Defective Items sent to workshop";
?>
<html>
	<head>	
		...
		<table  border="1" cellpadding="1" cellspacing="1"class="table table-bordered table-hover center-page" >
			...
			<?php
				...
				while ($emp=mysqli_fetch_array($records, MYSQLI_ASSOC)) {
					...
					$t = $_SESSION['priority'];
					if ($t > "1") {
						echo "<a href='change_defective_status.php?id=$emp[id]'>";
						echo $emp['status']."</a>";
