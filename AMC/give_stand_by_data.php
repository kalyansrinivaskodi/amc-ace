...
<?php
  if($_SESSION['login_success']==true){ 
	...
	$sql="SELECT * FROM usercomplaint WHERE Status LIKE 'standby' ORDER BY id DESC";
...
<html>
  ...
  <br>
		...
		<h4 class="display-page"> Stand By Pending Complaint Details</h4>
		...
		<table  border="1" cellpadding="1" cellspacing="1"class="table table-bordered table-hover center-page" >
			...
			<?php
				...
				while ($emp=mysqli_fetch_array($records, MYSQLI_ASSOC)) {
					...
					$t = $_SESSION['priority'];
					if ($t > "1") {
						echo "<a href='change_standby_status.php?id=$emp[id]'>";
						echo "standby</a>";
