...
if (isset($_GET['status'])) {
    ...
    function getIssuesByStatus($username, $status) {
        ...
        $sql = "SELECT * FROM usercomplaintsced where status='".$status."' ORDER BY id DESC";
...
<div class="issues-container">
    <center>
        <h2><?php echo ucfirst($status); ?> Issues<button class="button" onclick=printDiv()>Print Data</button>
                    ...
                    <tr>
                        ...
                        <td>                            
                            <?php if ($status === 'Pending'): ?>                       
                                <button onclick="console.log('Button clicked'); openModal('<?php echo $issue['id']; ?>',
                                 '<?php echo $issue['name']; ?>', '<?php echo urlencode($issue['description']); ?>', '<?php echo $issue['created_at']; ?>')">Change Status</button>                              
                            <?php endif; ?>
                        </td>
